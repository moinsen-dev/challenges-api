/**
 * Every operation this service offers, in one list.
 *
 ...
 *
 * It lives in a module of its own, importing nothing, so that both the OpenAPI
 * document and the documentation page on the website can read it. The page used
 * to keep its own hand-written list under a heading reading "Everything under
 * /v1"; by the time anybody counted it was showing 53 of 125 operations. A
 * second list of the same thing is a promise nobody can keep.
 */

export type Auth = 'public' | 'app' | 'secret' | 'player' | 'admin' | 'developer'

export type Entry = [method: string, path: string, auth: Auth, tag: string, summary: string]
